...
static volatile _Bool keyboardScanCodeValid = 0;
static volatile uint8_t keyboardData = 0;
static PS2ScanCode keyboardScanCode = {};
static volatile _Bool keyboardIsExtend = 0;
static volatile _Bool keyboardIsBreak = 0;
...
void PS2Keyboard_Init() {
    ...
    IOCAF4_SetInterruptHandler(PS2Keyboard_ClockInterrupt);
    IOCAF5_SetInterruptHandler(PS2Keyboard_DataInterrupt);
}
...
static void PS2Keyboard_ClockInterrupt(void) {
    ...
    if (keyboardCommandBits > 0) {
        if(keyboardCommandCount > 0 && keyboardCommandCount < 9) {
            ...
            _Bool bit = (keyboardCommand >> bitPos) & 1;
            if(bit) {
        ...
        if (SysTime_GetCurrentTimeMs() - prevTimeMs >= 250) {
            keyboardState = KeyboardStart;
        }
        switch (keyboardState) {
            ...
            case KeyboardParity:
                ...
                break;
            case KeyboardStop:
                if (keyboardScanCode.value == 0xE0) {
                    keyboardIsExtend = 1;
                } else if (keyboardScanCode.value == 0xF0) {
                    keyboardIsBreak = 1;
                } else {
