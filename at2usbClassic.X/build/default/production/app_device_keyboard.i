...
# 1 "./usb/usb_device_hid.h" 1
# 98 "./usb/usb_device_hid.h"
void USBCheckHIDRequest(void);
# 280 "./usb/usb_device_hid.h"
...
# 35 "app_device_keyboard.c" 2
# 50 "app_device_keyboard.c"
const struct {
    uint8_t report[63];
} hid_rpt01 = {
    { 0x05, 0x01,
...
static KEYBOARD_INPUT_REPORT inputReport __attribute__((address(0x0A0)));




static volatile KEYBOARD_OUTPUT_REPORT outputReport __attribute__((address(0x120)));
...
void APP_KeyboardTasks(const PS2ScanCode* scanCode) {
    signed int TimeDeltaMilliseconds;
    unsigned char i;
    _Bool needToSendNewReportPacket;


    APP_KeyboardUpdateState(scanCode);
    ...
    if (((keyboard.lastINTransmission != 0x0000) && ((*(volatile uint8_t*)keyboard.lastINTransmission & 0x80) != 0x00)) == 0) {
        ...
        GenericQueue_CopyToArray(&keyboard.keys, inputReport.keys);
        inputReport.modifiers.value = keyboard.modifiers.value;
...
static void APP_KeyboardProcessOutputReport(void) {
    keyboard.leds.value = outputReport.value;
    PS2Keyboard_SendCommand(0xED);
}
